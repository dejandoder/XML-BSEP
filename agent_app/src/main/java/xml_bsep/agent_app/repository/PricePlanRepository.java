...
public interface PricePlanRepository extends JpaRepository<PricePlan, Long> {
	...
	@Query("select count(*) from PricePlan pricePlan where (pricePlan.fromDate <= :toDate and pricePlan.toDate >= :toDate) or (pricePlan.fromDate <= :fromDate and pricePlan.toDate >= :fromDate) or (pricePlan.toDate >= :toDate and pricePlan.fromDate <= :fromDate)")
	public int checkIfPlanIsValid(@Param("fromDate") Date fromDate, @Param("toDate") Date toDate);
